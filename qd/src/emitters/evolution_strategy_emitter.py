...
Adapted from pyribs:
https://github.com/icaros-usc/pyribs/blob/master/ribs/emitters/_evolution_strategy_emitter.py
"""
import itertools
...
from src.emitters.opt import CMAEvolutionStrategy
...
class EvolutionStrategyEmitter(EmitterBase):
    ...
    Args:
        ...
        seed (int): Value to seed the random number generator. Set to None to
            avoid a fixed seed.
        constraint_func (callable): A function which takes in a batch of
            solutions and returns an array of booleans indicating whether each
            solution satisfies the given constraints.
    Raises:
    ...
    def tell(self,
        ...
        # TODO: What is the proper way to handle mu selection when the number of
        # solutions is less than expected? It was originally self._batch_size //
        # 2 but we switched it to len(solution_batch) // 2.
        num_parents = (new_sols if self._selection_rule == "filter" else
                       len(solution_batch) // 2)
