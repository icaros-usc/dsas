...
class CMAEvolutionStrategy:
    ...
    def ask(self, lower_bounds, upper_bounds, constraint_func):
        ...
        Args:
            ...
            constraint_func (callable): A function which takes in a batch of
                solutions and returns an array of booleans indicating whether
                each solution satisfies the given constraints.
        """
        ...
        while len(remaining_indices) > 0:
            ...
            satisfies_constraints = (np.ones(len(new_solutions), dtype=bool)
                                     if constraint_func is None else
                                     constraint_func(new_solutions))
            ...
            # out of bounds).
            failure_indices = (np.any(out_of_bounds, axis=1) |
                               ~satisfies_constraints)
            remaining_indices = remaining_indices[failure_indices]
