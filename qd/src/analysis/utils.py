...
def load_archive_from_history(logdir: LogDir, individual=False) -> GridArchive:
    ...
    Note that these archives will only contain objectives and measures.
    ...
    for gen_history in archive_history:
        for method, objective, measures in gen_history:
            if method == "add_single":
                archive.add_single([], objective, measures)
            elif method == "add":
                archive.add(np.empty((len(objective), 0)), objective, measures)
            if individual:
                yield archive
        if not individual:
            yield archive
