...
def collect(manifest: str, reps: int = None, output: str = "figure_data.json"):
    ...
    for env in figure_data:
        ...
        for algo in figure_data[env]:
            ...
            for entry in figure_data[env][algo]:
                ...
                for metric in reversed(entry_metrics):
                    ...
                    f = interp1d(np.array(evals)[x_data], entry_metrics[metric])
                    total_evals = evals[-1]
        ...
        for sans in [True, False]:
            with mpl_style_file("comparison_sans.mplstyle" if sans else
                ...
                markers = dict(zip(all_algos, itertools.cycle("ovD^XsP")))
...
def comparison_qd_score(
    figure_data: str = "./figure_data.json",
    output: str = "comparison_qd_score",
    palette_name: str = "colorblind_reordered",
    height: float = 2.75,
):
    """Plots figure comparing only QD score of all algorithms.
        ...
        for sans in [True, False]:
            with mpl_style_file("comparison_sans.mplstyle" if sans else
                ...
                for (row_val, col_val), ax in grid.axes_dict.items():
                    ...
                    if env == "Shared Workspace Collaboration dist_gp":
                        ax.set_xticks([0, 5000, 10000])
                        ax.set_xticklabels([0, "5k", "10k"])
                        ax.set_xlim(0, 10000)
                        ax.set_yticks([0, 60000, 120000])
                        ax.set_yticklabels([0, "60k", "120k"])
                        ax.set_ylim(0, 120000)
                    elif env == "Shared Workspace Collaboration path_overlap":
                        ax.set_xticks([0, 5000, 10000])
                        ax.set_xticklabels([0, "5k", "10k"])
                        ax.set_xlim(0, 10000)
                        ax.set_yticks([0, 10000, 20000])
                        ax.set_yticklabels([0, "10k", "20k"])
                        ax.set_ylim(0, 20000)
                    elif env == "Shared Autonomy":
                        ax.set_xticks([0, 5000, 10000])
                        ax.set_xticklabels([0, "5k", "10k"])
                        ax.set_xlim(0, 10000)
                        ax.set_yticks([0, 12000, 24000])
                        ax.set_yticklabels([0, "12k", "24k"])
                        ax.set_ylim(0, 24000)
                ...
                fig_width, fig_height = grid.fig.get_size_inches()
                #  legend_height = 1.1
                grid.fig.set_size_inches(fig_width + 1.0, fig_height)
                #  fig_height + legend_height)

                # Save the figure.
                grid.fig.tight_layout(
                    rect=(0, 0, 1,
                          0.99),  # fig_height / (fig_height + legend_height)),
                    # No extra whitespace on edges so that it fits well in the
                    # paper.
                    pad=0.0,
                )
                for extension in ["pdf", "png", "svg"]:
...
def comparison_qd_score_time(
    figure_data: str = "./figure_data.json",
    output: str = "comparison_qd_score_time",
    palette_name: str = "colorblind_reordered",
    height: float = 2.75,
):
    """Plots figure comparing QD score vs cumulative time of all algorithms.
    ...
    for env in figure_data:
        ...
        for algo in figure_data[env]:
            ...
            for entry in figure_data[env][algo]:
                entry_metrics = entry["series_metrics"]
                times = np.array(entry_metrics["Time"]) / 3600.0  # Hours.
                ...
                for metric in reversed(entry_metrics):
                    ...
                    # Perform interpolation.
                    f = interp1d(times,
                                 entry_metrics[metric])  # Assumes use_zero.
                    ...
                    # How frequently to plot points; e.g. plot every 0.2 hours.
                    plot_every = 0.2

                    # Plot fewer data points to reduce file size.
                    xnew = np.arange(0.0, total_time + 1e-6, plot_every)
                    metricnew = f(xnew)
                    data_len = len(metricnew)
            ...
            with mpl_style_file("comparison_sans.mplstyle" if sans else
                ...
                for (row_val, col_val), ax in grid.axes_dict.items():
                    ...
                    if env == "Shared Workspace Collaboration dist_gp":
                        ...
                        ax.set_xticks([0, 0.9, 1.8])
                        ax.set_xticklabels([0, "0.9", "1.8"])
                        ax.set_xlim(0, 1.8)
                        ax.set_yticks([0, 10000, 20000])
                        ax.set_yticklabels([0, "10k", "20k"])
                        ax.set_ylim(0, 20000)
...
def run_pairwise_ttests(figure_data, metric):
    ...
    if metric in ["Normalized QD Score", "Archive Coverage"]:
        h2_bonf_n = 4  # Comparing 2 algorithms to 2 algorithms.
        h2_baselines = ["CMA-MAE", "MAP-Elites"]
        return {
            "H1":
                compare_to("Random Search",
                           ["DQD-DSAGE", "DSAGE", "CMA-MAE", "MAP-Elites"]),
            "H2 DQD-DSAGE":
                compare_to("DQD-DSAGE", h2_baselines),
            "H2 DSAGE":
                compare_to("DSAGE", h2_baselines),
            "H3":
                compare_to("DQD-DSAGE", ["DSAGE"]),
        }
    else:
...
if __name__ == "__main__":
    fire.Fire({
        ...
        "comparison_qd_score": comparison_qd_score,
        "comparison_qd_score_time": comparison_qd_score_time,
        "table": table,
        "tests": tests,
    })
