...
def visualize_env_archives(mode,
    ...
    for col, (cur_ax, algo) in enumerate(zip(ax, viz_data[env]["Algorithms"])):
        ...
        transpose_measures = viz_data[env].get("transpose", False)

        # Set x label along bottom.
        # https://matplotlib.org/stable/gallery/subplots_axes_and_figures/figure_title.html
        cur_subfig.supxlabel(
            viz_data[env]["ylabel" if transpose_measures else "xlabel"])

        if mode == "heatmap":
            xbounds = viz_data[env].get("ybounds" if transpose_measures else "xbounds", [0,1])
            ybounds = viz_data[env].get("xbounds" if transpose_measures else "ybounds", [0,1])
            ...
            plot_generation_on_axis(
                ...
                gen=gen,
                plot_kwargs={
                    "aspect": (xbounds[1] - xbounds[0]) / (ybounds[1] - ybounds[0]),
                    "cmap": "viridis",
                    "pcm_kwargs": {
                        "rasterized": True,
                    },
                    "transpose_measures": transpose_measures,
                    "vmin": viz_data[env].get("min_obj", 0.0),
                    "vmax": viz_data[env].get("max_obj", 1.0),
                },
            )

            cur_ax.set_xticks(xbounds)
            cur_ax.set_yticks(ybounds)

            if col == 0:
                # Set y label on left plot.
                cur_ax.set_ylabel(
                    viz_data[env]["xlabel" if transpose_measures else "ylabel"])
            else:
            ...
            with plt.style.context(f):
                ...
                for env in viz_data:
                    fig = plt.figure(figsize=(
                        {
                            # Width.
                            "Shared Workspace Collaboration dist_gp": 7,
                            "Shared Workspace Collaboration path_overlap": 7,
                            "Shared Autonomy": 7,
                        }[env],
                        {
                            # Height.
                            "Shared Workspace Collaboration dist_gp": 1.75,
                            "Shared Workspace Collaboration path_overlap": 1.75,
                            "Shared Autonomy": 1.75,
                        }[env]))
