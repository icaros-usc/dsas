function Orbitline(options, shaderlibrary) {
	...
	this._shaderUniforms = { 
								lineWidth: {type: "f", value: options.linewidth || 0.02}, 
						    	diffuseColor: {type: "v4", value: new THREE.Vector4(1.0,1.0,1.0,1.0)}
						   };
	...
	if(options.linetex) {
		...
		this._shaderUniforms["hatchMap"] = {type: "t", value: options.linetex};
	} else {
...
Orbitline.prototype.updateGeometry = function(newpts) {
...
Orbitline.prototype.buildGeometry = function() {
	...
	for (var i = 0; i < npts; ++i ) {
		...
		// colorList.push(new THREE.Vector3(tval, tval, tval));
		colorList.push(new THREE.Vector3(1, 0.5, 0));
		colorList.push(new THREE.Vector3(1, 0.5, 0));
	}
