...
file_name = "MAPELITES_BC_sim20003_elites_freq20.csv"
BC_TYPE = "BC_obstacle_new_blend_2"
...
class Elite():
  def __init__(self, sc_id, p1_y, disturbances, obstacle_pos_y, fitness, cell_x, cell_y, cell_z, elite_map_config):
    ...
    #if cell_x == 6 and cell_y == 55 and cell_z == 1:
    ...
    for i,one_map in enumerate(all_records):
        if  i == 999:
          ...
          for data_point in one_map:
            ...
            #  continue
            elite = Elite(sc_id, p1_y, disturbances, obstacle_pos_y, fitness, cell_x, cell_y, cell_z, elite_map_config)
...
def find_elite(elites, cell_x, cell_y, cell_z):
    for i in range(0,len(elites)):
        #if elites[i].cell_x == cell_x and elites[i].cell_y == cell_y:
            ##from IPython import embed
            #embed()  
        if elites[i].cell_x == cell_x and elites[i].cell_y == cell_y and elites[i].cell_z == cell_z:
            return elites[i]
...
if __name__ == "__main__":
  ...
  parser.add_argument('-z','--cell_z', help='path of experiment cell_z file',type=int,required=True)
  opt = parser.parse_args()
  ...
  elite = find_elite(elites, cell_x, cell_y, cell_z)
  #from IPython import embed
  #embed()
