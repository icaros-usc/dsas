                ...
                #    map = np.zeros((gridsize,gridsize))
                   # print(i)
                    ...
                    for data_point in one_map: 
                        ...
                        if data_point_info == ['']:
                            break
                        bc_1 = float(data_point_info[12])
                        bc_2 = float(data_point_info[13])
...
if __name__ == "__main__":
    ...
    data_folder = str("../experiments/BC3")
    #data_root = 'test_file'
    data_root = data_folder + str("/ME")
    ...
    data_root = data_folder + str("/RANDOM")
    files = sorted([f for f in os.listdir(data_root)])[0:]
    #from IPython import embed
    #embed()
    trial_indx = 1
    ...
    data_root = data_folder + str("/CMAES")
    files = sorted([f for f in os.listdir(data_root)])[0:]
    #from IPython import embed
    #embed()
    trial_indx = 1
    ...
    data_root = data_folder + str("/CMAME")
    files = sorted([f for f in os.listdir(data_root)])[0:]
    #from IPython import embed
    #embed()
    trial_indx = 1
    ...
    data_root = data_folder + str("/ISOLINEDD")
    files = sorted([f for f in os.listdir(data_root)])[0:]
    #from IPython import embed
    #embed()
    trial_indx = 1
    ...
    #file_name = files[1]
      file_name = str(data_root + "/" + file_name)
      QD_score_file = import_data_file(file_name,feature_ranges, 'MAP-Elites (line)', trial_indx)
      trial_indx = trial_indx + 1


    summary_filename = data_folder + "/summary.csv"
