...
class GoalPredictor(object):
    ...
    def __init__(self, goals):
        ...
        self.log_goal_distribution = np.log(
            (1.0 / len(self.goals)) * np.ones(len(self.goals))
        )
    ...
    def clip_prob(self):
        ...
        if self.log_goal_distribution[max_prob_ind] > self.log_max_prob_any_goal:
            ...
            diff = (
                np.exp(self.log_goal_distribution[max_prob_ind]) - self.max_prob_any_goal
            )
            # want to distribute this evenly among other goals
            diff_per = diff / (len(self.log_goal_distribution) - 1.0)

            # distribute this evenly in the probability space...this corresponds to doing
            # so in log space
            # e^x_new = e^x_old + diff_per, and this is formulate for log addition
            self.log_goal_distribution += np.log(
                1.0 + diff_per / np.exp(self.log_goal_distribution)
            )
            # set old one
